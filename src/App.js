  ...
  return (
    <body>
      ...
      <ul>
        {lista.map((item, i) => {
          return <li>{item}
          <button onClick={() => setLista(lista.filter((value,index) => index !== i))}>Remover</button>
          <button onClick={() => setLista (lista.map((value,index) => index !== i ? value:name))}>Alterar</button>
          </li>;
        })}
      </ul>
    </body>
